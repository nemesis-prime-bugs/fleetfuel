...
export type FillUpRepo = {
    listByVehicle(vehicleId: Id): Promise<ListResult<FillUp & {
        receipts?: Receipt[];
    }>>;
...
export type TripRepo = {
    listByVehicle(vehicleId: Id): Promise<ListResult<(Trip & {
...
export type ProfileRepo = {
    ...
    patch(patch: Partial<Profile>): Promise<{
        email: string;
        profile: Profile;
    }>;
};
